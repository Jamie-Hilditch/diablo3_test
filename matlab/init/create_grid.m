...
% Select the type of grid function to be used
 ...
 GRID_TYPE = 3 %input('Select a grid type: ');
...
if (GRID_TYPE==1) 
  % Closed Channel
  for J=1:N+1
    G(J+1)=(Lz/2.0)*tanh(CS*((2.0*(J-1))/(N)-1.0))/tanh(CS);
  end
elseif (GRID_TYPE==2)
% Grid for a shear layer, with higher resolution in the center
  for J=1:N+1
    G(J+1)=(Lz/2.0)*(CS*((2.0*(J-1))/N-1.0)^3+(2.0*(J-1))/N-1.0)/(CS+1);
  end
elseif (GRID_TYPE==3)
  % Closed Channel
  for J=1:N+1
    G(J+1)=(Lz/2.0)*tanh(CS*((2.0*(J-1))/(N)-1.0))/tanh(CS)+Lz/2.0;
  end
elseif (GRID_TYPE==4)
% Surface boundary layer
  for J=1:N+1
    G(J+1)=Lz*tanh(CS*((N-(J-1))/N-1.0))/tanh(CS)+Lz;
  end
  G(:)=G(:)*-1;
else
